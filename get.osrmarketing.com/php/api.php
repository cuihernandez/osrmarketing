...
// Add this new section to handle the fine-tuned model:

if (isset($_POST['prompt'])) {
    $user_prompt = $_POST['prompt'];
}
$isFlowiseKB = ($model == 'FlowiseKB');

$fine_tuned_model_id = "file-pDA6hO5ht13SRqOrhP4lJtUs";
$org_id = "org-eaM5xPlCcQP3R6h3snWolqKt";

if ($model == $fine_tuned_model_id) {
    $model = "ft:gpt-3.5-turbo:$org_id:" . substr($fine_tuned_model_id, 5);
}

$fine_tuned_model_id = "file-Rw4Uh7AcHvXUSVhqnQUNH4kK";
if ($model == $fine_tuned_model_id) {
    $model = "ft:gpt-3.5-turbo:org-eaM5xPlCcQP3R6h3snWolqKt:Rw4Uh7AcHvXUSVhqnQUNH4kK";
}
// Use $response to send back to the chat UI
...
// When FlowiseKB is selected
if ($isFlowiseKB) {
    // setting params for curl request
    $header = [
        "Content-Type: application/json",
        "Authorization: Bearer UaC3FCLH9bm9bRTxjM7wQwjtZiAeNlylGidL0/zVwDo="
    ];
    $url = "https://mspprompts.onrender.com/api/v1/prediction/4bfe1969-317a-4850-8ef8-2f5ab838911c";
    $params = json_encode(['question' => $_POST['prompt']]);
    $chunk_buffer = '';
    $curl = curl_init($url);
    $options = [
        ...
        CURLOPT_WRITEFUNCTION => function ($curl, $data) use (&$chunk_buffer) {
            ...
            if ($httpCode != 200) {
                if ($httpCode == 500) {
                    ...
                    echo 'data: {"error": "[ERROR]", "message":"' . $data . '"}' . PHP_EOL;
                }
            } else {
                $chunk_buffer .= $data;
                echo $data;
    ...
    if ($response === false) {
        ...
        if ($isLogged) {
            $response_data = json_decode($chunk_buffer);
            $assistant_response = $response_data->text;
            $total_characters = strlen($assistant_response);

            // Save chatting to the database
            $_SESSION["history"][$ai_id][] = [
                "item_order" => $next_item_order,
                "id_message" => $id = md5(microtime()),
                "role" => "assistant",
